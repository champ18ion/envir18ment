...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (reason === "expired") setSessionMsg("Your session expired — sign in to continue");
    if (reason === "session") setSessionMsg("Your encryption key was cleared — sign in again to re-derive it");
  ...
  return (
    <div style={{ background: "#05060f", minHeight: "100vh", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 32px", borderBottom: "1px solid rgba(186,215,247,0.06)", position: "relative", zIndex: 1 }}>
        ...
        <Link href={inviteFlow ? "/register?invite=1" : "/register"} style={{ fontSize: "13px", color: "#9da7ba", textDecoration: "none" }}>Create account</Link>
      </nav>
        ...
        <form onSubmit={submit} style={{ width: "100%", maxWidth: "380px" }}>
          <div style={{ background: "rgba(5,6,15,0.97)", border: "1px solid rgba(186,215,247,0.12)", borderRadius: "16px", padding: "32px", boxShadow: "rgba(216,236,248,0.2) 0px 1px 1px 0px inset, rgba(168,216,245,0.06) 0px 24px 48px 0px inset, rgba(0,0,0,0.3) 0px 16px 32px 0px" }}>
            <div style={{ marginBottom: "24px" }}>
              <p style={{ fontSize: "11px", color: "#81899b", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Sign in to</p>
              <h1 className="font-heading e18-gradient-text" style={{ fontSize: "22px", fontWeight: 400 }}>envir18ment</h1>
            </div>

            {sessionMsg && (
              <div style={{ padding: "10px 12px", background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.2)", borderRadius: "6px", marginBottom: "16px", fontSize: "13px", color: "#fbbf24" }}>
                {sessionMsg}
              </div>
            )}
            {error && (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "20px" }}>
              ...
              <div>
                ...
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} required placeholder="••••••••" className="e18-input" />
              </div>
            </div>
            ...
            <p style={{ marginTop: "20px", textAlign: "center", fontSize: "13px", color: "#81899b" }}>
              ...
              <Link href={inviteFlow ? "/register?invite=1" : "/register"} style={{ color: "#d1e4fa", textDecoration: "none" }}>Sign up</Link>
            </p>
          </div>
        </form>
      </main>
